<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    $role = auth()->user()->role;
@endphp
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 bg-blue-500 dark:bg-blue-700">
            ...
            <div class="flex">
                ...
                <div class="shrink-0 text-white dark:text-white-800 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center">
                        ...
                        <div class="ml-2">
                            <div class="font-bold text-sm">{{ App\Models\Setting::first()->webname }}</div>
                            <div class="text-sm">{{ App\Models\Setting::first()->description }}</div>
                        </div>
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link 
                        ...
                        class="text-gray-700 dark:text-gray-200 transition-all duration-300 ease-in-out hover:text-white hover:bg-gradient-to-r from-indigo-500 to-purple-500 px-3 py-1 rounded-lg">
                        {{ __('Dashboard') }}
                    </x-nav-link>
                </div>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                            <div>({{ Auth::user()->jurusan }} {{ Auth::user()->identitas }}) - {{ Auth::user()->nama }}</div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            <div class="px-4">
                <div class="font-medium text-base text-gray-800 dark:text-gray-200">({{ Auth::user()->identity }}) {{ Auth::user()->nama }} - {{ Auth::user()->jurusan}}</div>
