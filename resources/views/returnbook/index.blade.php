                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($pengembalian as $item)
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 flex flex-wrap gap-2">
                                            @if(auth()->user()->role !== 'mahasiswa')
                                                <a href="{{ route('pengembalian-buku.edit', $item->id) }}"
                                                    class="inline-block bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold px-3 py-1 rounded shadow transition duration-200">
                                                    Edit
                                                </a>
                                                ...
                                                <x-confirm-delete-modal>
                                                    <x-slot name="trigger">
                                                        <button @click="isOpen = true"
                                                            class="inline-block bg-red-500 hover:bg-red-600 text-white text-xs font-semibold px-3 py-1 rounded shadow transition duration-200">
                                                            Hapus
                                                        </button>
                                                    </x-slot>

                                                    <x-slot name="title">Konfirmasi Hapus</x-slot>
                                                    <x-slot name="content">Apakah Anda yakin ingin menghapus pengembalian ini?</x-slot>
                                                    <x-slot name="footer">
                                                        <form id="deleteForm-{{ $item->id }}"
                                                            action="{{ route('pengembalian-buku.destroy', $item->id) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
